...
"""
    apply_operator!(working_memory, target, source, operator, boost, compress) ->
...
If the `boost` argument is given, the vector's effective norm is multiplied by that number
without affecting the expected value of the result. If the `compress` argument is passed,
stochastic vector compression is not performed after the step.
...
"""
function apply_operator!(working_memory, target, source, ham, boost=1, compress=true)
    v = localpart(source)
