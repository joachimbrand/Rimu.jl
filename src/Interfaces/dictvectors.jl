...
"""
    apply_operator!(working_memory, target, source, operator) ->
...
`target` and `working_memory` may be mutated and/or swapped.
"""
function apply_operator!(working_memory, target, source, ham)
    v = localpart(source)
    ...
    for (add, val) in pairs(v)
        spawn_stats += apply_column!(working_memory, ham, add, val)
    end
