...
import ..DictVectors: working_memory
...
struct PDVecThreading <: ThreadingStrategy end

function working_memory(::PDVecThreading, t::PDVec)
    return PDWorkingMemory(t)
end
function fciqmc_step!(::PDVecThreading, w::PDWorkingMemory, ham, src::PDVec, shift, dτ)
...
"""
select_threading_strategy(t::ThreadingStrategy, _, _) = t

function select_threading_strategy(threading::Symbol, targetwalkers, v)
    if v isa PDVec
        return PDVecThreading()
    elseif threading == :auto
        t = targetwalkers > 500 && Threads.nthreads() > 1
        return select_threading_strategy(t, targetwalkers, v)
    else
