...
dv = MPIData(v; setup = RMPI.mpi_one_sided, capacity = 500)
@mpi_root @show dv.s
...
post_step = ProjectedEnergy(ham, localpart(dv))
...
el = @elapsed nt = lomc!(ham,dv; params, s_strat, post_step, threading=false)
...
savefile = joinpath(@__DIR__,"mpi_df_os.arrow") # using the Arrow data format relying on Arrow.jl
