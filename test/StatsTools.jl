...
@testset "ratio_of_means" begin
    ...
    qr95 = MonteCarloMeasurements.pquantile(real(r.ratio), [0.025,0.975])
    @test qr95[1] < 0 < qr95[2]
    qi95 = MonteCarloMeasurements.pquantile(imag(r.ratio), [0.025,0.975])
    @test qi95[1] < 0 < qi95[2]
    @test_throws ErrorException MonteCarloMeasurements.pquantile(r, [0.025,0.975])
    ...
    @test isapprox(
        [f-σ_f, f, f+σ_f], MonteCarloMeasurements.pquantile(amc / bmc, [0.16,0.5,0.84]),
        rtol = 2/√2000
    )
    ...
    @test isapprox(μ_a/μ_b, MonteCarloMeasurements.pmedian(r.ratio); atol = 2σ_f)
    q = MonteCarloMeasurements.pquantile(r.ratio, [0.16,0.5,0.84])
    ...
    @test isapprox(q, MonteCarloMeasurements.pquantile(rs.ratio, [0.16,0.5,0.84]); atol = 2σ_f)
    ...
    @test isapprox(μ_a/μ_b, MonteCarloMeasurements.pmedian(r); atol = abs(2σ_f))
    qr = MonteCarloMeasurements.pquantile(real(r.ratio), [0.16,0.5,0.84])
    qi = MonteCarloMeasurements.pquantile(imag(r.ratio), [0.16,0.5,0.84])
...
@testset "Reweighting" begin
    ...
    @test 0.2 < MonteCarloMeasurements.pmedian(pcb_est) < pcb
    ...
    @test MonteCarloMeasurements.pmedian(bp) ≈ bp_intervals.ratio
