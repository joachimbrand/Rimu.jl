...
@testset "lomc!/QMCState" begin
    ...
    @testset "Replicas" begin
        ...
        @testset "AllOverlaps" begin
            ...
            df, _ = lomc!(H, dv; replica=AllOverlaps(2; operator=(GutzwillerSampling(H, 1), H)))
            @test num_stats(df) == 3 * binomial(2, 2)
            df, _ = lomc!(H, dv; replica=AllOverlaps(7; operator=(GutzwillerSampling(H, 1), H)))
            @test num_stats(df) == 3 * binomial(7, 2)

            # Transformed operators: (3 + 1) * N choose 2 reports.
            df, _ = lomc!(H, dv; replica=AllOverlaps(2; operator=(H, GutzwillerSampling(H, 1)), transform=GutzwillerSampling(H, 0.5)))
            @test num_stats(df) == 4 * binomial(2, 2)
            df, _ = lomc!(H, dv; replica=AllOverlaps(7; operator=(H, GutzwillerSampling(H, 1)), transform=GutzwillerSampling(H, 0.5)))
